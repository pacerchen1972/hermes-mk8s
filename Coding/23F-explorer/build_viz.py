...
function filterSearch(query) {
  ...
  if (matches.length === 1) showDetail(matches[0]);
}
...
function initTimeline() {
  ...
  // Y-jitter: bucket by hour (golpe) or date (others)
  const bucketKey = d => def.hourly
    ? (d.date || '').slice(0, 13)   // 'YYYY-MM-DDTHH'
    : (d.date || '').slice(0, 10);  // 'YYYY-MM-DD'
  const buckets = d3.group(periodEvents, bucketKey);
  const yJitter = new Map();
  buckets.forEach(evs => {
    evs.forEach((ev, i) => {
      yJitter.set(ev, (i - (evs.length - 1) / 2) * 12);
    });
  });
  ...
  svg.selectAll('.tl-event')
    ...
    .join('g')
      ...
      .attr('transform', d => {
        ...
        const xp = isNaN(dt.getTime()) ? margin.left
                 : Math.max(margin.left, Math.min(W - margin.right, x(dt)));
        return `translate(${xp},${centerY + (yJitter.get(d) || 0)})`;
      })
    .call(g => g.append('circle')
      .attr('r', 5)
      .attr('fill', d => periodColor[d.period] || '#8b949e')
      .attr('opacity', 0.85)
      .style('cursor', 'pointer')
      .on('mouseover', (event, d) => showTimelineTooltip(event, d))
      .on('mouseout', hideTooltip)
      .on('click', (event, d) => {
        if (d.doc_id) {
          const node = GRAPH_DATA.nodes.find(n => n.doc_id === d.doc_id);
